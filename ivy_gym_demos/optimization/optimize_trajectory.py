...
def train_step(compiled_loss_fn, env, optimizer, initial_state, logits):
    initial_state = ivy.to_native(initial_state, nested=True)
    loss, grads = ivy.execute_with_gradients(lambda lgts: compiled_loss_fn(env, initial_state, lgts['l']),
...
def main(env_str, steps=100, iters=10000, lr=0.1, seed=0, log_freq=100, vis_freq=1000, visualize=True, f=None, fw=None):
    ...
    for iteration in range(iters):
        ...
        score, logits = train_step(loss_fn, env, optimizer, starting_state, logits)
        if iteration == 0:
