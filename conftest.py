...
import ivy
import ivy.ivy_tests.test_ivy as ivy_tests

FW_STRS = ['numpy', 'jax', 'tensorflow', 'torch', 'mxnet']


TEST_FRAMEWORKS: Dict[str, callable] = {'numpy': lambda: ivy_tests.helpers.get_ivy_numpy(),
                                        'jax': lambda: ivy_tests.helpers.get_ivy_jax(),
                                        'tensorflow': lambda: ivy_tests.helpers.get_ivy_tensorflow(),
                                        'torch': lambda: ivy_tests.helpers.get_ivy_torch(),
                                        'mxnet': lambda: ivy_tests.helpers.get_ivy_mxnet()}
TEST_CALL_METHODS: Dict[str, callable] = {'numpy': ivy_tests.helpers.np_call,
                                          'jax': ivy_tests.helpers.jnp_call,
                                          'tensorflow': ivy_tests.helpers.tf_call,
                                          'torch': ivy_tests.helpers.torch_call,
                                          'mxnet': ivy_tests.helpers.mx_call}


@pytest.fixture(autouse=True)
def run_around_tests(dev_str, f, wrapped_mode, compile_graph, call):
    if wrapped_mode and call is ivy_tests.helpers.tf_graph_call:
        # ToDo: add support for wrapped_mode and tensorflow compilation
        pytest.skip()
    if wrapped_mode and call is ivy_tests.helpers.jnp_call:
        # ToDo: add support for wrapped_mode with jax, presumably some errenously wrapped jax methods
        pytest.skip()
    if 'gpu' in dev_str and call is ivy_tests.helpers.np_call:
