...
def test_demo_run_through(env, device, fw):
    ...
    main(env, visualize=False, fw=fw)


@pytest.mark.parametrize(
    "env", ["CartPole", "Pendulum", "MountainCar", "Reacher", "Swimmer"]
)
def test_demo_optimize_trajectory(env, device, fw):
    if fw in ["numpy", "tensorflow_graph"]:
        # numpy does not support gradients,
        # and demo compiles already, so no need to use tensorflow_graph
        pytest.skip()
    ...
    main(env, steps=1, iters=1, lr=0.1, seed=0, visualize=False, fw=fw)


@pytest.mark.parametrize(
    "env", ["CartPole", "Pendulum", "MountainCar", "Reacher", "Swimmer"]
)
def test_demo_optimize_policy(env, device, fw):
    if fw in ["numpy", "tensorflow_graph"]:
        # numpy does not support gradients,
        # and demo compiles already, so no need to use tensorflow_graph
        pytest.skip()
